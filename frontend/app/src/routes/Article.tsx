...
import CreateModal from '../components/articleComponents/CreateModal'
import DeleteModal from '../components/articleComponents/DeleteModal'
...
function Article() {
  ...
  const delet = async (id: any) => {
    axios.delete(`http://localhost:8081/Artículo/${selected}`,{
        headers: {
  ...
  const onSubmit = async (data: any) => {
    await fetch('http://localhost:8081/Artículo', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  const find = async () => {
    axios.get('http://localhost:8081/Artículo',{
        headers: {
          'Content-Type': 'application/json'
        },
        method: 'GET',
