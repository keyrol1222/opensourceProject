...
const UpdateModal: React.FC<UpdateProductModalProps> = ({ onClose, onSubmit }) => {
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const formData: FormData = {
      name: (event.target as any).name.value,
      brand: (event.target as any).brand.value,
      price: parseInt((event.target as any).price.value),
